...
class CotacoesList(generics.ListAPIView):
    """_
    Classe view que representa a API do projeto
    """
    ...
    def get_queryset(self):
        ...
        if dt_inicio and dt_fim:
            resultado = resultado.filter(
                data__gte=converter_data(dt_inicio),
                data__lte=converter_data(dt_fim),
                moeda__in=["EUR", "JPY", "BRL"],
            )
        elif dt_inicio:
            resultado = resultado.filter(
                data__gte=converter_data(dt_inicio), moeda__in=["EUR", "JPY", "BRL"]
            )
        elif dt_fim:
            resultado = resultado.filter(
                data__lte=converter_data(dt_inicio), moeda__in=["EUR", "JPY", "BRL"]
            )
